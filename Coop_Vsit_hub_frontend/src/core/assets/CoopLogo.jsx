 ...
 * Prominent, large brand logo with a clean, subtle `CoopBank • Visit Hub` subtitle.
 */
    ...
    <div className={`flex flex-col items-center justify-center text-center select-none ${className}`}>
      ...
      {!hideText && variant !== 'icon' && (
        <div className={`mt-2 font-sans font-bold tracking-widest uppercase text-slate-500 flex items-center justify-center gap-1 ${currentSize.text}`}>
          <span className="text-slate-800 font-black">CoopBank</span>
          <span className="text-slate-400">•</span>
          <span className="text-[#00adef] font-black">Visit Hub</span>
        </div>
      )}
    </div>
  );
