...
import { z } from 'zod';
import { Building2, Mail, Sparkles, Check, Edit2 } from 'lucide-react';
...
export const EditOrganizationModal = () => {
  const { isEditModalOpen, closeEditModal, updateOrganization, editingOrg, organizations } =
  ...
  useEffect(() => {
    if (editingOrg) {
      const sector = editingOrg.industrySector || '';
      ...
      reset({
        ...
        primaryContactPerson: editingOrg.primaryContactPerson || editingOrg.contactPersonName || '',
        marketCountry: editingOrg.marketCountry || 'Ethiopia',
        relationshipScore: editingOrg.relationshipScore || editingOrg.relationshipHealthScore || 85,
        overviewNotes: editingOrg.overviewNotes || editingOrg.notes || '',
      });
    }
  }, [editingOrg, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    await updateOrganization(editingOrg.id, {
      ...data,
      contactPersonName: data.primaryContactPerson?.trim() || undefined,
  ...
  return (
    <Modal
      ...
      subtitle="Update corporate details, contact person, and strategic intelligence."
      maxWidth="max-w-2xl"
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-left">
        ...
        <Input
          ...
          placeholder="e.g. Ethio Telecom, Visa Inc."
